import AsyncStorage from "@react-native-async-storage/async-storage";
import React, { useState } from "react";
import {
  Pressable,
  ScrollView,
  Text,
  View,
  useWindowDimensions,
  type NativeScrollEvent,
  type NativeSyntheticEvent,
} from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";
import { Ionicons } from "@expo/vector-icons";
import appJson from "../../../app.json";
import { SEARCH_RADIUS_M } from "../../config";
import { useTheme } from "../../theme";
import { useStyles } from "./styles";

export const AGE_VERIFIED_KEY = "age_verified_v1";

type Props = {
  onVerified: () => void;
};

type Page = {
  icon?: keyof typeof Ionicons.glyphMap;
  title: string;
  body: string[];
  isAgeGate?: boolean;
};

const RADIUS_KM = SEARCH_RADIUS_M / 1000;

const PAGES: Page[] = [
  {
    title: "Compass",
    body: [
      "Find the nearest liquor store",
      "This app locates nearby stores using your device's location and compass. No purchases are made through this app.",
      "By continuing, you confirm you are of legal drinking age in your jurisdiction.",
    ],
    isAgeGate: true,
  },
  {
    icon: "beer-outline",
    title: "Beer Seeker",
    body: [
      "A compass for the nearest drink.",
      "Point the phone, follow the needle, arrive thirsty.",
    ],
  },
  {
    icon: "people-outline",
    title: "About Us",
    body: [
      "We're Ben, Sam and Josh — and we created This Beer Seeker App.",
      "We're massive history buffs. One fact we love was that in the Middle Ages most people couldn't read, so they navigated by landmarks instead - a sign had to be vivid enough for anyone to recognise at a glance, which is why England still has The Blue Boar, The White Hart and The King's Head.",
      "A habit the Romans started, hanging vine leaves outside a tavern to show it sold wine.",
      "That's where Beer Seeker came from. We built it as a return to true form for travelling and looking for pubs",
    ],
  },
  {
    icon: "compass-outline",
    title: "The Compass",
    body: [
      "The needle reads your device's magnetometer and swings to the closest liquor store, turning with you as you walk.",
      "Tap the map underneath for a closer look, or switch between the Modern and Classic faces in Settings.",
    ],
  },
  {
    icon: "list-outline",
    title: "Browse",
    body: [
      `Every bar, pub, sports bar, brewery, wine bar and bottle shop within ${RADIUS_KM} km, sorted by distance.`,
      "Filter by type or search by name, then open a venue for its rating, opening hours and phone number.",
    ],
  },
];

export const OnboardingScreen: React.FC<Props> = ({ onVerified }) => {
  const [page, setPage] = useState(0);
  const [ageVerified, setAgeVerified] = useState(false);
  const { width } = useWindowDimensions();
  const { colors } = useTheme();
  const styles = useStyles();

  const onScrollEnd = (e: NativeSyntheticEvent<NativeScrollEvent>) => {
    if (ageVerified) {
      setPage(Math.round(e.nativeEvent.contentOffset.x / Math.max(width, 1)));
    }
  };

  const confirm = async () => {
    await AsyncStorage.setItem(AGE_VERIFIED_KEY, "true");
    setAgeVerified(true);
  };

  const currentPage = PAGES[page];
  const isAgeGatePage = currentPage?.isAgeGate && !ageVerified;

  return (
    <SafeAreaView style={styles.safeArea} edges={["top"]}>
      {isAgeGatePage ? (
        <View style={styles.ageGateContainer}>
          <Text style={styles.ageGateTitle}>{currentPage.title}</Text>
          <Text style={styles.ageGateSubtitle}>{currentPage.body[0]}</Text>

          <View style={styles.divider} />

          {currentPage.body.slice(1).map((line) => (
            <Text key={line} style={styles.ageGateBody}>
              {line}
            </Text>
          ))}

          <Pressable style={styles.confirmBtn} onPress={confirm}>
            <Text style={styles.confirmText}>I am of legal age — Enter</Text>
          </Pressable>
        </View>
      ) : (
        <>
          <View style={styles.header}>
            <Text style={styles.headerTitle}>About</Text>
            <View style={styles.dotsContainer}>
              {PAGES.slice(1).map((_, i) => (
                <View key={i} style={[styles.dot, i === page - 1 && styles.dotActive]} />
              ))}
            </View>
          </View>

          <ScrollView
            horizontal
            pagingEnabled
            showsHorizontalScrollIndicator={false}
            onMomentumScrollEnd={onScrollEnd}
            scrollEventThrottle={16}
            style={styles.pager}
          >
            {PAGES.slice(1).map((p, idx) => (
              <ScrollView
                key={p.title}
                style={{ width }}
                contentContainerStyle={styles.page}
                showsVerticalScrollIndicator={false}
              >
                {p.icon && (
                  <View style={styles.iconRing}>
                    <Ionicons name={p.icon} size={30} color={colors.primary} />
                  </View>
                )}
                <Text style={styles.pageTitle}>{p.title}</Text>
                <View style={styles.pagesDivider} />
                {p.body.map((line) => (
                  <Text key={line} style={styles.pageBody}>
                    {line}
                  </Text>
                ))}

                {idx === PAGES.length - 2 && (
                  <Pressable style={styles.getStartedBtn} onPress={onVerified}>
                    <Text style={styles.getStartedText}>Get Started</Text>
                  </Pressable>
                )}
              </ScrollView>
            ))}
          </ScrollView>

          <Text style={styles.footnote}>Version {appJson.expo.version}</Text>
        </>
      )}
    </SafeAreaView>
  );
};
